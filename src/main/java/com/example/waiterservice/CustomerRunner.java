...
public class CustomerRunner implements ApplicationRunner {
  ...
  private void showServiceInstances() {
    discoveryClient.getInstances("consul-waiter-service").forEach(
  ...
  private long orderCoffee() {
    ...
    RequestEntity<OrderRequest> request = RequestEntity
        .post(UriComponentsBuilder.fromUriString("http://consul-waiter-service/order/").build(new HashMap<>()))
        .body(orderRequest);
  ...
  private void queryOrder(Long orderId) {
    URI uri = UriComponentsBuilder
        .fromUriString("http://consul-waiter-service/order/{id}")
        .build(orderId);
